...
#define SIO_NAME_STR16  L"Nuvoton NCT5585D"
UINT16  HwmBase = 0;
...
)
{
	...
	if (GetDeviceID() == 0x5570) {
		...
		Print(L"\nLDN 0x04 (System Wake-Up Control)\nIDX ");
		...
		Print(L"\nLDN 0x0f (Shared Memory/Flash Interface)\nIDX ");
		for (i = 0; i < sizeof(registerAddressesF) / sizeof(registerAddressesF[0]); i++) {
			Print(L"%02x ", registerAddressesF[i]);
		}
		Print(L"\nVAL ");
		...
		Print(L"\nLDN 0x13 (Serial Peripheral Interface)\nIDX ");
		...
		Print(L"\nLDN 0x14 (Platform Environment Control Interface)\nIDX ");
		for (i = 0; i < sizeof(registerAddresses14) / sizeof(registerAddresses14[0]); i++) {
			Print(L"%02x ", registerAddresses14[i]);
		}
		Print(L"\nVAL ");
		...
		Print(L"\nLDN 0x18 (Power Management Interface Channel 4)\nIDX ");
...
)
{
	...
	if (GetDeviceID() == 0x5571) {
		...
		Print(L"\nPage 4\n");
		for (j = 0x00; j <= 0xAB; j++) {
			Bram = BramPageAddress(0x04, j);
			Print(L"[%02x]: %d\n", j, Bram);
		}

		Print(L"\nPage 7\n");
		for (j = 0x00; j < 0xFF; j++) {
			Bram = BramPageAddress(0x07, j);
			Print(L"[%02x]: %d\n", j, Bram);
		}
		Bram = BramPageAddress(0x07, 0xFF);
		Print(L"[FF]: %d\n", Bram);

		Print(L"\nPage 14\n");
		Bram = BramPageAddress(0x14, 0x70);
		Print(L"[70]: %d\n", Bram);
	}
}
...
)
{
	...
	if (EFI_ERROR(Status)) {
		if ((Status == EFI_VOLUME_CORRUPTED) && (ProblemParam != NULL)) {
			ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN(STR_GEN_PROBLEM), gShellDebug1HiiHandle, L"nctinfo", ProblemParam);
		...
		if (ShellCommandLineGetCount(Package) > 1) {
			ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN(STR_GEN_TOO_MANY), gShellDebug1HiiHandle, L"nctinfo");
			ShellStatus = SHELL_INVALID_PARAMETER;
			goto Done;
		}
	}
